...
from matrix_utils import exact_matrix_logarithm_trace, power_series_matrix_logarithm_trace, power_series_matrix_logarithm_trace_JT, Chebyshev_matrix_logarithm_trace_JT
import pdb
...
class conv_iresnet_block(nn.Module):
    ...
    def forward(self, x, ignore_logdet=False, trace_estimator="power"):
        ...
        #     trace = power_series_matrix_logarithm_trace(Fx, x, self.numSeriesTerms, self.numTraceSamples)
            ...
            if trace_estimator == "power":
                trace = power_series_matrix_logarithm_trace(Fx, x, self.numSeriesTerms, self.numTraceSamples, seed=0)
            elif trace_estimator == "power-JT":
                trace = power_series_matrix_logarithm_trace_JT(self.bottleneck_block, x, self.numSeriesTerms, self.numTraceSamples, seed = 0)
            elif trace_estimator == "chebyshev-JT":
                trace = Chebyshev_matrix_logarithm_trace_JT(self.bottleneck_block, x, self.numSeriesTerms, self.numTraceSamples, seed=0)
            elif callable(trace_estimator):
                trace = trace_estimator(Fx, x, self.numSeriesTerms, self.numTraceSamples)
            else:
...
class conv_iResNet(nn.Module):
    ...
    def forward(self, x, ignore_logdet=False, trace_estimator = "power"):
        ...
        traces = []
        for block in self.stack:
            z, trace = block(z, ignore_logdet=ignore_logdet, trace_estimator = trace_estimator)
            traces.append(trace)
...
if __name__ == "__main__":
    ...
    print("ResNet Shape:", resnet.final_shape)
    ...
    # === Approximate trace  ===
    trace_methods = ["power", "power-JT", "chebyshev-JT"]

    for method in trace_methods:
        print(f"\n=== Running trace estimator: {method} ===")
        start_time = time.time()
        z, logpz, tr = resnet(x, trace_estimator=method)
        duration = time.time() - start_time
        print(f"Trace ({method}):", tr.detach())
        print(f"Time ({method}) : {duration:.4f} seconds")
